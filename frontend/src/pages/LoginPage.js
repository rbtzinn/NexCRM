...
export default function LoginPage() {
  const { login } = useAuth();
  const { language, setLanguage, t, translateApiError } = useI18n();
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      toast.error(typeof msg === "string" ? translateApiError(msg) : translateApiError());
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-black flex">
      ...
      >
        ...
        <div className="relative z-10 flex flex-col justify-between p-12">
          ...
          <div>
            ...
            <p className="text-zinc-500 text-sm mt-4">{t("login.quoteAuthor")}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 lg:max-w-[480px] flex flex-col justify-center px-8 sm:px-12 lg:px-16">
        <div className="flex items-center justify-between mb-10">
          ...
          <div className="flex items-center bg-[#0A0A0A] border border-white/[0.08] rounded-lg p-0.5 ml-auto">
            {["pt", "en"].map((lang) => (
              <button
                key={lang}
                type="button"
        ...
        <div className="w-full max-w-sm mx-auto">
          <div className="mb-8">
            <h1 className="font-outfit text-3xl font-semibold text-white tracking-tight">{t("login.signIn")}</h1>
            <p className="text-zinc-500 text-sm mt-2">{t("login.subtitle")}</p>
          </div>
          ...
          <div className="mt-8">
            ...
            {publicDemoMode && (
              <div className="mb-3 rounded-lg border border-white/[0.08] bg-[#0A0A0A] px-3.5 py-3">
                <p className="text-xs font-medium text-zinc-300">{t("login.publicDemoTitle")}</p>
                <p className="text-[11px] text-zinc-500 mt-1 leading-relaxed">
                  {t("login.publicDemoDescription")}
                </p>
              </div>
            )}
              ...
              {demoAccounts.map((acc) => (
                ...
                >
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="text-xs font-medium text-zinc-300">{t(`login.roles.${acc.role}`)}</span>
